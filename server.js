...
**********************/
// REMEMBER to chagne the password
...
app.get('/team_stats', function(req,res) {
	...
	var wins = 'select * from football_games where home_score > visitor_score;';
	var losses = 'select * from football_games where home_score < visitor_score;';
	db.task('get-everything', task => {
		return task.batch([
			task.any(games),
			task.any(wins),
...
app.get('/player_info/select_player', function(req,res){
  ...
  var playerInfo = "select * from football_players where id = " + player_id + ";";
  var gamesPlayed = "select count(*) from football_games where (" + player_id + ") = any(players);";
  db.task('get-everything', task => {
    return task.batch([
      task.any(players),
      task.any(playerInfo),
  ...
  .then(data => {
    res.render('pages/player_info',{
      ...
      playersAll: data[0],
      playerInfos: data[1][0],
      allGamesPlayed: data[2][0],
